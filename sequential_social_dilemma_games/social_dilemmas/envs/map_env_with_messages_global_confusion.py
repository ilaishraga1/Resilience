...
class MapEnvWithMessagesAndGlobalRewardPrediction(MapEnv):
    def __init__(
            self,
            ascii_map,
            extra_actions,
            ...
            obs_space = {
                **obs_space,
                ...
                "other_agent_predicted_rewards": Box(
                    low=-self.max_reward_value, high=self.max_reward_value, shape=(self.num_agents - 1,),
                    dtype=np.float32,
                ),
                "other_agent_actual_rewards": Box(
                    low=-self.max_reward_value, high=self.max_reward_value, shape=(self.num_agents - 1,),
                    dtype=np.float32,
                ),
            }
            ...
            for agent in self.agents.values():
                ...
                observations[agent.agent_id] = {
                    **observations[agent.agent_id],
                    ...
                    "other_agent_actual_rewards": others_actual_rewards_for_current_step,
                }
    ...
    def reset(self):
        ...
        if self.use_messages_attribute:
            ...
            others_predicted_rewards_for_current_step = np.array([0 for _ in range(self.num_agents - 1)]).astype(
                np.float32)
            others_actual_rewards_for_current_step = np.array([0 for _ in range(self.num_agents - 1)]).astype(
                np.float32)
            for agent in self.agents.values():
                observations[agent.agent_id] = {
                    **observations[agent.agent_id],
